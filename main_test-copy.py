...
def main():
    ...
    # KAGGLE NOTE: Ensure this points to /kaggle/input/your-dataset/... 
    parser.add_argument('--file_video_input', type=str,
                        help='input audio/video file (with extension: .mp4, .avi, .wav).')
    # KAGGLE NOTE: Point to the features computed from gen_references, or /kaggle/input/ if pre-computed
    parser.add_argument('--dir_poi', type=str,
                        help='features directory of POI.')
    # Modality fixed to onlyaudio to ensure video logic is bypassed
    parser.add_argument('--modality', type=str, default='onlyaudio',
                        help="the modality to use. Fixed to 'onlyaudio'.")
    ...
    # KAGGLE NOTE: Must be stored in /kaggle/working/ so you can download it later
    ...
    if os.path.isfile(file_output + '.npz') and argd.skip_if_exists:
        ...
        # compute local scores (Audio-Only)
        method = create_model(logger, argd.dir_poi, opt, device=device, only_audio=True)
        
        if argd.modality == 'onlyaudio':
            dict_out, info_tracks = method.compute_distance_audio(argd.file_video_input, verbose=argd.verbose)
        else:
            print(f'ERROR: Modality {argd.modality} is not defined or supported in this branch!')
            exit()
